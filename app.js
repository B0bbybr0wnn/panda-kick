...
function renderAvatarHTML(container, avatarId, hasPhoto, photo) {
  if (hasPhoto && photo) {
    ...
    container.innerHTML = AVATARS[avatarId] || AVATARS.ball;
  }
}
...
function refreshHomeUI() {
  ...
  renderAvatarHTML(el.avatarBtn, state.avatar, !!state.photo, state.photo);
  renderAvatarHTML(el.profileAvatar, state.avatar, !!state.photo, state.photo);

  // small initial fallback (only if no avatar)
  if (!state.photo) {
    el.avatarBtn.innerHTML = AVATARS[state.avatar] || AVATARS.ball;
    el.profileAvatar.innerHTML = AVATARS[state.avatar] || AVATARS.ball;
  }
...
function renderAvatarPicker() {
  ...
  // Free avatars first, then premium
...
el.editProfile.addEventListener("click", () => {
  ...
  el.photoHint.textContent = state.premium
    ? "Premium · Tap upload to change your photo"
    : "Premium only · Go premium to upload your photo";
  renderAvatarPicker();
...
el.premiumBuy.addEventListener("click", () => {
  ...
  refreshHomeUI();
  play(el.sfxCorrect);
  alert("Premium unlocked! +500 coins bonus. All 15 premium avatars unlocked.");
  showScreen("profile");
});
...
el.diffSwitch.addEventListener("click", () => {
  ...
  el.currentDiff.style.background =
    state.difficulty === "easy" ? "var(--gold)" : "var(--purple)";
  play(el.sfxTap, false); buzz(20);
  renderQuestion();
});
...
window.addEventListener("load", () => {
  ...
  // Enter sound ~1s after app launch
  setTimeout(() => {
    try {
